<template>
  <div class="ta-list-page">
    ...
    <div class="ta-list">
      <div v-for="item in list" :key="item.historyId" class="ta-list-item" @click="goRead(item)">
        <div class="ta-list-cover">
          <img v-if="item.chapterCover || item.mangaCover" :src="item.chapterCover || item.mangaCover" alt="" />
          <div v-else class="ta-list-cover-ph">📖</div>
        </div>
        <div class="ta-list-info">
          <div class="ta-list-title">{{ item.mangaName }}</div>
          <div class="ta-list-sub">{{ item.chapterName }}</div>
          <div class="ta-list-time">{{ item.updateTime || '' }}</div>
        </div>
      </div>
    </div>
    <div v-if="list.length === 0" class="ta-empty">暂无记录</div>
  </div>
</template>
...
onMounted(async () => {
  try {
    const res = await historyApi.get(1, 50)
    list.value = res?.list || []
  } catch (e) { /* empty */ }
})
...
.ta-list-cover-ph { width: 100%; height: 100%; display: flex; align-items: center; justify-content: center; font-size: 18px; background: linear-gradient(135deg, #e0e7ff, #c7d2fe); }
...
.ta-list-time { font-size: 11px; color: #9ca3af; margin-top: 4px; }
