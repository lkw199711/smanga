  ...
  <div class="ta-manage-page">
    ...
    <div class="ta-table-wrap">
      <table class="ta-table">
        ...
        <tbody>
          <tr v-for="(row, idx) in list" :key="row.mangaId">
            <td><input type="checkbox" :value="row.mangaId" v-model="selected" /></td>
          ...
          <tr v-if="list.length === 0"><td colspan="8" class="ta-empty">暂无漫画</td></tr>
        </tbody>
      </table>
    </div>
...
function toggleAll(e: Event) {
  selected.value = (e.target as HTMLInputElement).checked ? list.value.map(r => r.mangaId) : []
}
