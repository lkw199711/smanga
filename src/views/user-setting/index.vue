<template>
  <div class="user-setting">
    ...
    <div class="form-note mt-4 text-gray-500 text-sm mb-6">请注意，设置调配完成后，需要点击保存按钮才能够提交保存。否则有些配置项不能够生效。</div>
    <el-form :model="form" label-width="140px" class="setting-form">
      <el-card class="setting-card mt-6" shadow="hover">
        ...
        <el-form-item label="语言设置" class="setting-item">
          <el-select v-model="userConfig.language" class="setting-select" size="default" @change="language_change">
        ...
        <el-form-item label="主题设置" class="setting-item theme-setting">
          ...
          <el-select v-model="userConfig.theme" class="setting-select" size="default" @change="theme_change">
            <el-option v-for="item in theme" :key="item.value" :label="$t(`theme.${item.value}`)" :value="item.value">
      ...
      <el-card class="setting-card mt-6" shadow="hover">
        ...
        <el-form-item label="章节默认排序" class="setting-item">
          <el-select v-model="userConfig.chapterOrder" class="setting-select" size="default" @change="sort_order_change">
        ...
        <div class="form-note mt-4 text-gray-500 text-sm">
          自定义页面容量设置，漫画与章节的页面容量已根据不同尺寸设备调试配置好。
          <br />
          如果还是未能很好的适配您的设备，例如条目内容过多遮挡住分页器与页码，可以对页面容量进行自定义。
          <br />
          设置为0时，新主题会根据列表元素与当前布局尺寸自动计算；旧主题继续使用设备默认值。
          <br />
          (该设置仅在客户端生效，不影响其他设备)
        </div>
      </el-card>
      ...
      <el-card class="setting-card mt-6 mb-6" shadow="hover">
        ...
        <div class="form-note mt-4 text-gray-500 text-sm mb-4">
          开启此选项后，章节阅读界面将显示一个进度条用以切换页码，而不是传统的页码器。
          <br />
          此改动可以减少视觉干扰，并且可以更快的切换到目标页码。
          <br />
          但会使部分用户不适应，并且失去传统页码器的一些功能。
        </div>

		<el-form-item label="条漫加载步进" class="setting-item">
        ...
        <div class="form-note mt-4 text-gray-500 text-sm mb-4">
          开启此选项后，点击媒体库时会直接进入章节列表页，而不是漫画详情页。
          <br />
          此改动可以减少一次点击操作，可以看到阅读进度条。
          <br />
        ...
        <div class="form-note mt-4 text-gray-500 text-sm mb-4">
          开启此选项后，条漫模式下会自动加载所有图片，而不是只加载当前屏幕可见的图片。
          <br />
        ...
        <div class="form-note mt-4 text-gray-500 text-sm mb-4">
          开启此选项后，smanga加载章节时会将所有的图片裁剪文件名中的数字部分拼接在一起，按照大小升序排序。
          <br />
          不开启此选项时，smanga会按照图片文件名以ASCII进行排序。
          <br />
        </div>
        ...
        <div class="form-note mt-4 text-gray-500 text-sm mb-4">
          开启此选项后，章节列表将只显示章节名称，不再显示阅读进度与章节封面等信息。
          <br />
          此改动可以减少视觉干扰，在界面中展示更多的章节。
          <br />
        </div>
