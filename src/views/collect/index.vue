<template>
  <div class="search">
    <div class="top">
      <tabs v-model="collectStore.collectType" :items="items" @change="tabs_change"></tabs>
    </div>
    ...
    <div class="middle">
      <div class="touch-dom">
        ...
        <template v-else>
          <div class="manga-list" v-if="collectStore.collectType === 'manga'">
            ...
            <media-pager
              ...
              :page="page"
              :count="count"
              :page-size="browse.mangaListPageSize"
              :page-size-config="browse.mangaListPageSizes"
              @page-change="page_change" />
          </div>

          <div class="chapter-list" v-if="collectStore.collectType === 'chapter'">
            <!--章节列表-->
...
import {computed, watch, onMounted, ref, onActivated} from 'vue';
...
import useCollectStore from './collect-store';
...
function get_page_size_array() {
  ...
  const pageSizesConfig = collectStore.collectType === 'manga' ? mangaPageSize : chapterPageSize;
...
async function page_change(pageParams = 1, pageSize: number = 10) {
  ...
  // 缓存页码
  if (collectStore.collectType === 'manga') {
...
function context_menu(itemInfo: any) {
  if (collectStore.collectType === 'chapter') {
