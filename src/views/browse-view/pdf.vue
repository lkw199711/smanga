<template>
    <div class="browse-view">
        ...
        <div v-if="config.android" @click="switch_menu" id="flowList" ref="flowList">
            <VuePdfEmbed annotation-layer text-layer :source="browseStore.pdfPath" @loaded="handleDocumentLoad" />
        </div>
...
const pdfSource = '/118.pdf' // 支持URL、Base64或Uint8Array

// 处理文档加载完成
const handleDocumentLoad = (pdf) => {
    // 保存总页数到状态管理
    //   browseStore.pageCount = pdf.numPages;
};
...
async function change_chapter(chapterId: number) {
    const chapter = browseStore.chapterList.find((item) => item.chapterId === chapterId);
    browseStore.page = 1;
    browseStore.imagePathList = [];
    browseStore.imageFileList = [];
    await router.push({
...
browseStore.browseType = 'pdf';
// 生命周期
onMounted(() => {
    imagesLoaderRef.value.finish_loader();
    // 设置浏览模式
