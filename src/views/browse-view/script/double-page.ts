...
export default defineComponent({
	...
	methods: {
		...
		async page_change(page: number) {
			...
			// 有缓存则加载缓存的图片
			if (this.imgPathFiles[index]) {
				this.imgSrc1 = this.imgPathFiles[index];
				this.imgSrc2 = this.imgPathFiles[index + 1]
					? this.imgPathFiles[index + 1]
					: '';
				return;
			}
		...
		remove_poster() {
			if (this.removeFirst) {
                ...
                this.imgPathFiles.unshift(this.firstImageFile);
			} else {
                this.firstImage = this.imgPathList.shift() || '';
                this.firstImageFile = this.imgPathFiles.shift() || '';
			}
