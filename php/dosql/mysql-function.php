...
function dosql($params){
	...
	$cond = @getValue($params['cond'],array());
	...
	$cond = is_string($cond) ? json_decode($params['cond'],true) : $cond;
	...
	if(isset($cond['value'])){
		...
		foreach ($cond['value'] as $key => $value) {
			if (!is_keyword($value)) {
				...
				$cond['value'][$key] = mysql_real_escape_string($value);
			}
		}
		return $cond['value'];
	}
...
function format_parameter($parameter){
	...
	if(isset($params[$parameter])) {
		$cond[$parameter] = $params[$parameter];
		#如果为字符串美型 则解码
		if(is_array(json_decode($cond[$parameter]))) {
			$cond[$parameter] = json_decode($params[$parameter]);
		}
	}
